local options = setmetatable({}, {__call = function (self, ...)
	return self.pop(...)
end})
...
local function matcher(option)
	local len, sign = #option, sub(option, 1, 1)
	if len == 0 then
		option = '[^=]*$'
	elseif sign ~= '-' and sign ~= '+' and sub(option, -1, -1) ~= '=' then
...
function options.last(option, opts)
...
function options.all(option, opts)
...
function options.pop(optstring, opts, args)
	if not args then opts, args = {}, opts end
	if not args then args = arg end -- the global one

	local signs = '-'
	if sub(optstring, 1, 1) == '+' then -- ksh getopts(1)
		signs, optstring = '-+', sub(optstring, 2)
	end

	local i, n, err = 1, #args, nil
	while i <= n do
		local sign, arg = sub(args[i], 1, 1), sub(args[i], 2)
		...
		while #arg > 0 do -- quadratic but error exits are simple
			local opt = sub(arg, 1, 1)
			local j = find(optstring, opt, 1, true)
			if not j then break end -- unknown option

			arg = sub(arg, 2)
			if sub(optstring, j+1, j+1) == ':' then
				if arg == '' and sub(optstring, j+1, j+2) ~= '::' then
